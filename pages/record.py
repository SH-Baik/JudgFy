...
from app.modules.recording_module import extract_decision_elements  # ✅ 모듈 경로에 맞게 수정
    ...
    if user_input.strip():
        ...
        if "error" in result:
                ...
                try:
                    with open(DATA_PATH, "r", encoding="utf-8") as f:
                        ...
                        if not isinstance(history, list):
                            st.warning("⚠️ 저장 파일이 리스트가 아닙니다. 초기화합니다.")
                            history = []
                except json.JSONDecodeError:
                    st.warning("⚠️ JSON 파싱 오류. 초기화합니다.")
                    history = []
            else:
                history = []
            ...
            if isinstance(history, list):
                ...
                st.error("❌ 판단 저장 실패: 내부 구조 오류")
    else:
        st.warning("⚠️ 판단 상황을 입력해주세요.")
